...
use std::{cmp::max, num::Wrapping};

use crate::{gen::Gen, parse};
...
fn test_ints() {
    ...
    for i in (0u8..=255).map(Wrapping) {
        // main 0:0 == N print;
        let code = format!("main 0:0 == {} pop", i);
        let input = vec![];
        let output = vec![i];
...
fn test_hex() {
    ...
    for i in (0u8..=255).map(Wrapping) {
        // main 0:0 == N print;
        let code = format!("main 0:0 == {:#04x} pop", i);
        println!("{}", code);
        let input = vec![];
        let output = vec![i];
        single_test(code, input, output);
...
#[test]
fn test_while() {
    let code = format!("main 0:0 == 0 pop 1 [0 neq] [print dup +] while");
    let input = vec![];
    let output = vec![
        Wrapping(0),
        Wrapping(1),
